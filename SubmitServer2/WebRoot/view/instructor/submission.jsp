...
<body>
	...
	<ss:studentPicture />
	<h2>
		${student.fullname}, Submission # ${submission.submissionNumber},
	...
	<p>
		...
		<a href="${studentViewLink}">Student view</a> &nbsp;|&nbsp; <a
			href="${grantExtensionLink}"> Grant extension </a>
	...
	(Currently: ${studentSubmitStatus.extension} hours)
		...
		<c:if test="${testRunList != null}">
			<c:url value="submissionAllTests.jsp" var="submissionAllTestsLink">
				<c:param name="submissionPK" value="${submission.submissionPK}" />
			</c:url>
	...
	<c:url var="markBrokenLink"
			...
			<c:otherwise>
				<p>
					This submission has been marked broken. It will never be tested by
					a buildServer unless it is specifically marked for retest by the
					'retest' option.
...
result, the submission is flagged as broken.
				...
				<ul>
					...
					<c:if test="${testOutcomeCollection.valueReleaseTests > 0}">
						<li>${testOutcomeCollection.valueReleaseTestsPassed}/${
							testOutcomeCollection.valueReleaseTests} points for release test
							cases.
					</c:if>
					<c:if test="${testOutcomeCollection.valueSecretTests > 0}">
						<li>${testOutcomeCollection.valueSecretTestsPassed}/${
							testOutcomeCollection.valueSecretTests} points for secret test
							cases.
					</c:if>
				</ul>
					...
					<table>
						...
						<c:if test="${cardinalCoverageStats != null}">
							<tr>
								...
								<td><a href="${cardinalLink}"> Public/Release/Secret
										tests </a></td> ${cardinalCoverageStats.HTMLTableRow}
							</tr>
						</c:if>
						...
						<c:if test="${releaseUniqueStats != null}">
							<tr>
								...
								<td><a href="${releaseUniqueLink}"
									title="covered by a release test but not by any public or student test">
										Unique Release </a><br></td> ${releaseUniqueStats.HTMLTableRow}
							</tr>
						</c:if>
						<c:if test="${intersectionCoverageStats != null}">
							<tr>
								...
								<td><b> (Public U Release) <br>intersect<br>
										(Public U Student)
								</b></td> ${intersectionCoverageStats.HTMLTableRow}
							</tr>
						</c:if>
					</table>
				</c:if>
		...
		<c:if test="${testRunList != null}">
			...
			<p>
				<a href="${submissionAllTestsLink}"> See other test results for
					this submission </a>
		</c:if>
