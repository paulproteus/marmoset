...
<html>
<ss:head
	title="Assign points for project ${project.title} in ${course.courseName}" />
...
<body>
	...
	<form class="form" name="inputForm"
		action="<c:url value="/action/instructor/AssignPoints"/>"
		method="POST">
		<h2>Testing setup</h2>
		<ul>
			<li>Testing setup tested <fmt:formatDate
					value="${testSetup.datePosted}" pattern="E',' dd MMM 'at' hh:mm a" />
				<c:set var="comment">
					<c:out value="${testSetup.comment}" />
				</c:set>
			<li>Test-setup comment: <input type="text" name="comment"
				value="${comment}" size="30" />
			<li>Solution submitted by <c:out
					value="${studentRegistration.classAccount}" /> at <fmt:formatDate
					value="${submission.submissionTimestamp}"
					pattern="E',' dd MMM 'at' hh:mm a" /> <c:if
					test="${testSetup.status == 'FAILED'}">
...
Starting with point totals <a href="$testRunLink}"> from test setup
						<c:out value="${canonicalTestSetup.description}" />. posted <fmt:formatDate
							value="${canonicalTestSetup.datePosted}"
...
No previous activated test setup
		...
		<table>
			...
			<c:forEach var="outcome"
				items="${testOutcomeCollection.allScoredOutcomes}"
				varStatus="counter">
				<c:if test="${outcome.testType != 'BUILD'}">
					<tr class="$r{counter.index % 2 == 1 ? 'odd' : 'even'}">
						...
						<td><input type="text" name="${outcome.testName}"
							class="pointValue" size="3"
							value="${canonicalTestOutcomeMap[outcome.testName].pointValue}"
							onchange="change()" /></td>
					</tr>
				</c:if>
			</c:forEach>
			<tr>
				...
				<td><input type="text" name="total" size="3"
					readonly="readonly"></td>
			</tr>
			<tr class="submit">
				<td class="label" colspan=4><input type=submit
					value="Assign points, edit test-setup comment and activate test setup"></td>
			</tr>
