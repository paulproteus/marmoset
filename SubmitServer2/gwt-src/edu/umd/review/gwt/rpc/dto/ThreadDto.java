...
/**
 * DTO for a thread. A thread belongs to a single file, and has 0 or more
 * published comments, and 0 or 1 draft comments.
 * 
 * @author rwsims@umd.edu (Ryan W Sims)
 */
public class ThreadDto implements Result, Comparable<ThreadDto> {
  private static final Logger logger = Logger.getLogger(ThreadDto.class.getName());
  
  private @CodeReviewThread.PK
  int id;
  private String file;
  private int line;
  ...
  @Nullable
  private RubricEvaluationDto rubricEvaluation;
  ...
  /**
   * Set the draft for this thread. The draft can be null, which indicates that
   * a preexisting draft has been published. Cannot set a draft on a thread that
   * already has one.
   */
  public void setDraft(CommentDto draft) {
    if (draft != null) {
      ...
      if (this.draft != null) {
        logger.warning("Duplication drafts " + this.draft.getId() + " and " + draft.getId());
  ...
  /**
   * Return the active draft for this thread authored by the current user, if
  ...
  /**
   * Set the rubric for this thread; it is an error to set a rubric for a thread
  ...
  /**
   * Indicate whether the thread needs a response (an acknowledge comment) from
   * a given reviewer. A reviewer may respond to a thread if all of the
   * following hold:
   * <ol>
   ...
   * <li>The last published comment in the thread is not authored by the
   * reviewer.
   * </ol>
   */
  ...
  public int compareTo(ThreadDto that) {
    return ComparisonChain.start().compare(this.line, that.line).compare(this.timestamp, that.timestamp)
